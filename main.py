...
# Load color history from JSON file
def load_history():
    global colorList
    if os.path.exists(HISTORY_FILE):
        try:
            ...
            colorList = []  # If file corrupted, reset
    else:
        colorList = []

# Save color history to JSON file
def save_history():
    with open(HISTORY_FILE, "w") as f:
        json.dump(colorList, f, indent=4)

# Print all colors in memory
def printColorList():
    print("Colors detected are:", end=" ")
    for color in colorList:
        print(f"#{color}", end=" ")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        print("1. Start capturing colors")
        print("2. Export colors to a file")
        print("3. Show total stored colors")
